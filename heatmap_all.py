...
def compact_data(d, days, hours):
    ...
    # BUT, we should calculate Alpha values for all day-hour values
    # based on all values, all values 
...
# now create totals for each day of week
# and add to dict
for day in days:
    result[day]['total'] = {}
    PV = 0
...
# ie items i total pv, total uv, 8am pv, 8am UV, etc.
new_result = compact_data(result, days, hours)
print("=======\n", new_result)
utils_io.put_file(json.dumps(new_result), 'heatmap_compact_data.txt', ['data_out'])

html = template('heat_maps.html', data=new_result, hours=hours, days=days)
print("=======\n", html)
...
# build html with bottle template and data (result)
