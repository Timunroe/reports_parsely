...
files = {
    "spectator": {
        "monthly": 'Site-Stats-Over-Time-Oct-01-2018-Oct-31-2018-thespec-com.csv',
    ...
    "record": {
        "monthly": 'Site-Stats-Over-Time-Oct-01-2018-Oct-31-2018-therecord-com.csv',
    ...
    "standard": {
        "monthly": 'Site-Stats-Over-Time-Oct-01-2018-Oct-31-2018-stcatharinesstandard-ca.csv',
    ...
    "examiner": {
        "monthly": 'Site-Stats-Over-Time-Oct-01-2018-Oct-31-2018-thepeterboroughexaminer-com.csv',
    ...
    "star": {
        "monthly": 'Site-Stats-Over-Time-Oct-01-2018-Oct-31-2018-thestar-com.csv',
...
monthly_file_path = 'reports/monthly/' + files[site]['monthly']
posts_file_path = 'reports/monthly/' + files[site]['posts']
...
# MAIN
monthly_values = utils.return_csv(monthly_file_path)
ma_values = utils.return_csv(ma_file_path)
posts_values = utils.return_csv(posts_file_path)
data = utils.site_stats(monthly_values, ma_values, ma_units)

# posts = utils.post_stats()
s = ''
s += f'Monthly web report: {site.title()} for month starting {monthly_values["Date"]}' + nl + nl
s += dbl_line
s += "TOP POSTS: by Total Engaged Minutes" + nl + sngl_line

for rank, item in enumerate(posts_values[:10], start=1):
...
s += f", about {utils.percentage(data['postv']['new'], data['pagev']['new'])}% of month's page views" + nl + dbl_line
